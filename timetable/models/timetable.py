...
class TimeTable(models.Model):
	...
	def default_get(self,vals):
		res = super(TimeTable, self).default_get(vals)
		res['program_id'] = self.env['standard.standard'].search([])[0].id
		return res
	
	@api.onchange('program_id','school_id','medium_id')
	def onchange_program_id(self):
		values = []
		if self.program_id and self.school_id and self.medium_id:
			class_ids = self.env['school.standard'].search([('standard_id','=',self.program_id.id),('school_id','=',self.school_id.id),('medium_id','=',self.medium_id.id)])
			for a_id in class_ids:
				values.append((0,0,{'standard_id':a_id.id,'start_date':a_id.start_date, 'end_date':a_id.end_date, 'teacher_id':a_id.teacher_id.id,'room_number':a_id.division_id.name}))
			print ("=============values",values)
   			self.one_year_timetable_ids = values
	...
	one_year_timetable_ids = fields.One2many('time.table.line', 'one_year_table_id', 'TimeTable')
...
class TimeTableLine(models.Model):
	...
	one_year_table_id = fields.Many2one('time.table', 'TimeTable')
